...
@Component({
  selector: 'app-sidebar-informacion-turista',
  templateUrl: './sidebar-informacion-turista.component.html',
  styleUrls: ['./sidebar-informacion-turista.component.css']
})
export class SidebarInformacionTuristaComponent implements OnInit {
		...
		this._publicationService.newPublication(this.token, this.publication).subscribe(
			response => {
				if(response.publicationStored){
					...
					this._router.navigate(['/turista/publicaciones']);					
				}else{
					this.status = 'error';
				}
