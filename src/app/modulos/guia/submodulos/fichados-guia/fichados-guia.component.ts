...
@Component({
  selector: 'app-fichados-guia',
  templateUrl: './fichados-guia.component.html',
  styleUrls: ['./fichados-guia.component.css']
})
export class FichadosGuiaComponent implements OnInit {
	...
	constructor(
		...
		this.title    = "Usuarios fichados por ti";
		this.url      = GLOBAL.url;
				...
				/*if(response.user.nick && (this.identity._id != response.user._id)){
					...
					if(!response.follows){
						...
						if(!adding){
							...
							if(page > this.pages){
								this._router.navigate(['/guia/home']);
							}	
						}
