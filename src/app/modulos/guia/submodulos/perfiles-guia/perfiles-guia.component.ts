...
@Component({
  selector: 'app-perfiles-guia',
  templateUrl: './perfiles-guia.component.html',
  styleUrls: ['./perfiles-guia.component.css']
})
export class PerfilesGuiaComponent implements OnInit {
			...
			response => {
				if(!response.users){
					...
					if(page > this.pages){ //--> si se pone una página incorrecta nos lleva a la primera
						this._router.navigate(['/guia/perfiles/',1]);
					}
				}
			},
			error => {
