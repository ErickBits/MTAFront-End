...
@Component({
  selector: 'app-sidebar-informacion-guia',
  templateUrl: './sidebar-informacion-guia.component.html',
  styleUrls: ['./sidebar-informacion-guia.component.css']
})
export class SidebarInformacionGuiaComponent implements OnInit {
		...
		this._publicationService.newPublication(this.token, this.publication).subscribe(
			response => {
				if(response.publicationStored){
					...
					this._router.navigate(['/guia/publicaciones']);					
				}else{
					this.status = 'error';
				}
