...
@Component({
  selector: 'app-mensajes-enviados-admin',
  templateUrl: './mensajes-enviados-admin.component.html',
  styleUrls: ['./mensajes-enviados-admin.component.css']
})
export class MensajesEnviadosAdminComponent implements OnInit {
			...
			response => {
				if(response.messages){
					...
					if(page > this.pages){ //--> si se pone una página incorrecta nos lleva a la primera
						this._router.navigate(['/admin/mensajesAdmin/', 1]);
					}
				}
			},
			error => {
